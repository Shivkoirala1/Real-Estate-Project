...
const InitEmiModal = ({ sale, saleLoading, saleError, submitting, onClose, onSubmit }) => {
  ...
  useEffect(() => {
    if (!sale) return;
    const p = Math.max(Number(sale.agreedPrice || 0) - Number(sale.downPaymentAmount || 0), 0);
  ...
  const eligible = sale && sale.status === 'verified' && sale.paymentType === 'emi';
  const ineligibilityReason = sale && !eligible
  ...
  const handleSubmit = (e) => {
    ...
    if (!Number.isFinite(p) || p <= 0) return setError('Principal amount must be greater than 0.');
    if (!Number.isInteger(t) || t < 1 || t > 360) return setError('Tenure must be a whole number between 1 and 360 months.');
    if (!Number.isFinite(inst) || inst <= 0) return setError('Installment amount must be greater than 0.');
    if (!startDate) return setError('Start date is required.');

    setError('');
    onSubmit({ principalAmount: p, tenureMonths: t, installmentAmount: inst, startDate });
  };

  return (
    <div className="fixed inset-0 z-[200] flex items-center justify-center px-4" role="dialog" aria-modal="true" aria-labelledby="init-emi-title">
      ...
      <div className="relative bg-white rounded-sm shadow-lifted w-full max-w-lg max-h-[90vh] overflow-y-auto p-6">
        <div className="flex items-start justify-between mb-4">
          <div>
            ...
            <h2 id="init-emi-title" className="font-display text-xl text-navy">Initialize EMI Plan</h2>
          </div>
        ...
        {saleError ? (
          ...
          sale && (
            <div className="bg-parchment/60 border border-navy/10 rounded-sm p-4 mb-5">
              <p className="font-medium text-navy">{sale.property?.title || 'Property'}</p>
              ...
              <div className="flex flex-wrap gap-x-6 gap-y-1 mt-2 text-xs text-slate-muted">
                <span>Agreed price: <span className="font-medium text-navy">{npr(sale.agreedPrice)}</span></span>
                <span>Down payment: <span className="font-medium text-navy">{sale.downPaymentAmount ? npr(sale.downPaymentAmount) : '—'}</span></span>
              </div>
            </div>
          )
        )}
          ...
          <form onSubmit={handleSubmit}>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="sm:col-span-2">
                <label className="label-field" htmlFor="emi-principal">Principal Amount (NPR) *</label>
                <input
                  id="emi-principal"
                  type="number"
                  min="0"
              ...
              <div>
                <label className="label-field" htmlFor="emi-tenure">Tenure (months) *</label>
                <input
                  id="emi-tenure"
                  type="number"
                  min="1"
              ...
              <div>
                <label className="label-field" htmlFor="emi-installment">Installment Amount (NPR) *</label>
                <input
                  id="emi-installment"
                  type="number"
                  min="0"
                  ...
                  className="input-field"
                  value={installment}
                  onChange={(e) => { setTouched(true); setInstallment(e.target.value); }}
                  placeholder="Flat amount per installment"
                  required
                />
                <p className="text-[11px] text-slate-muted mt-1.5 leading-relaxed">
                  Flat amount per installment — negotiate freely; no auto-amortization is imposed.
                </p>
                {touched && Number(principal) > 0 && Number(tenure) > 0 && (
                  <button
                    ...
                    onClick={() => {
                      setTouched(false);
                      setInstallment(String(Math.round(Number(principal) / Number(tenure))));
                    }}
              ...
              <div className="sm:col-span-2">
                <label className="label-field" htmlFor="emi-start">Start Date *</label>
                <input
                  id="emi-start"
                  type="date"
                  className="input-field"
...
const EmiPlans = () => {
  ...
  // Init flow: ?new=<saleId> arrives from My Sales after a verified EMI sale
  useEffect(() => {
    if (!saleId) {
      setSale(null);
      setSaleError('');
    ...
    (async () => {
      try {
        ...
        if (!cancelled) setSale(res.data.sale || null);
      } catch (err) {
        if (!cancelled) setSaleError(err.response?.data?.message || 'Failed to load the sale.');
      } finally {
  ...
  const handleCreatePlan = async (payload) => {
    setSubmitting(true);
    try {
      await api.post('/emi-plans', { saleId, ...payload });
      showToast('EMI plan created', 'success');
      closeInit();
      setPage(1);
      load();
    } catch (err) {
      showToast(err.response?.data?.message || 'Failed to create EMI plan', 'error');
    } finally {
      setSubmitting(false);
    }
  };
    ...
    <div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4 mb-8">
        <div className="bg-white border border-navy/10 rounded-sm p-5 shadow-card">
          ...
          <p className="text-3xl font-display text-navy">{initialLoaded ? summary.activePlans : '—'}</p>
        </div>
        <div className="bg-white border border-navy/10 rounded-sm p-5 shadow-card">
          <p className="text-xs uppercase tracking-wide text-slate-muted mb-2">Due This Month</p>
          <p className="text-3xl font-display text-navy">{initialLoaded ? summary.dueThisMonth : '—'}</p>
        </div>
        <div className="bg-white border border-navy/10 rounded-sm p-5 shadow-card">
      ...
      {loading ? (
        ...
        <div className="bg-white border border-navy/10 rounded-sm py-16 px-6 text-center">
          <p className="text-slate-muted">
            No EMI plans yet. Plans are initialized from a verified sale with payment type EMI (see My Sales).
          </p>
        </div>
      ) : plans.length > 0 ? (
        <>
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {plans.map((plan) => (
                  ...
                  >
                    <td className="px-5 py-4 font-medium text-navy">{plan.buyer?.name || '—'}</td>
                    <td className="px-5 py-4 text-slate-ink">{plan.property?.title || '—'}</td>
                    ...
                    <td className="px-5 py-4 font-medium text-navy">{npr(plan.outstandingBalance)}</td>
            ...
            {plans.map((plan) => (
              ...
              >
                <div className="flex items-center justify-between gap-2 mb-1.5">
                  <p className="font-medium text-navy">{plan.buyer?.name || '—'}</p>
                  {statusCell(plan)}
                </div>
                <p className="text-sm text-slate-ink mb-2">{plan.property?.title || '—'}</p>
                <div className="flex items-center justify-between gap-3 text-xs text-slate-muted">
                  <span>
                    Next due:{' '}
                    {plan.nextDueInstallment ? (
                      <span className="text-slate-ink">{nextDueText(plan)}</span>
                    ) : (
                      <span className="text-sage font-medium">All settled</span>
                    )}
                  </span>
                  <span>
                    Outstanding: <span className="text-navy font-medium">{npr(plan.outstandingBalance)}</span>
                  </span>
                </div>
              </div>
            ))}
