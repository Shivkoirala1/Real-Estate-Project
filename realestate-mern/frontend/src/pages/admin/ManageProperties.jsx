...
import { getProperties, getMyListings, updatePropertyStatus, deleteProperty } from '../../services/propertyService';
...
const ManageProperties = ({ showHeader = true }) => {
  ...
  const load = async () => {
    setLoading(true);
    try {
      const data = isAdmin ? await getProperties({ limit: 100 }) : await getMyListings();
