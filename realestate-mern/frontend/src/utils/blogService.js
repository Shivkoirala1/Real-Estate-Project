...
const API_BASE = "/api/blogs"; // adjust to match your axios instance / base URL setup
...
export const getPublishedBlogs = async ({ page = 1, limit = 9 } = {}) => {
  const { data } = await axios.get(`${API_BASE}/published`, {
    params: { page, limit },
  });
  return data; // { blogs, pagination }
};
...
export const updateBlog = async (id, { title, body, tags, status, coverImage }) => {
  ...
  const { data } = await axios.patch(`${API_BASE}/${id}`, formData, {
    headers: { "Content-Type": "multipart/form-data" },
  });
  return data; // { message, blog }
};
